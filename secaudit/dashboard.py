import base64,json,secrets,mimetypes,re
...
def serve(root,port=8765):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def authorized(self,mutation=False):
            ...
            if mutation and (self.headers.get('Origin') not in (f'http://127.0.0.1:{port}',f'http://localhost:{port}') or not secrets.compare_digest(self.headers.get('X-CSRF-Token',''),csrf)):
                self.send_error(403);return False
            return True
        ...
        def do_GET(self):
            ...
            except (ValueError,OSError): self.send({'error':'Requested artifact unavailable'},code=404)
        def do_POST(self):
